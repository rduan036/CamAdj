...
%     hold off;
    ...
    e_point = abs(e(1:2:end)) + abs(e(2:2:end));
    CamAdj_para = [0.001, max(e_point), 0.01, max(depths), 0.01, max(depths)];
    singular_idx = [];
    if length(e_point) > 10
        [~, check_idx] = mink(e_point, 10);
        singular_idx = CamAdjFiltering_R8(target_point2D(:, check_idx), PointsReproject(:, check_idx), orient, t, CamAdj_para);
        singular_idx = check_idx(singular_idx);
    else
        singular_idx = CamAdjFiltering_R8(target_point2D, PointsReproject, orient, t, CamAdj_para);
    end
    % CamAdj filtering
    if ~isempty(singular_idx)
        singular_idx = singular_idx - 1;
        singular_point = [singular_idx.*2 + 1; singular_idx.*2 + 2];
    end
    % compute control law
